...
tdcli_function ({
  ...
  if not lang then
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "_success_", 0, "md")
else
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "_به تمام ادمینها و مدیر گروه اجازهء فرمان دادن به ربات داده شد _", 0, "md")
     end
 end
...
local lang = redis:get(hash)
  ...
  if not lang then
    ...
    return 'ابتدا گروه را به لیست مدیریتی ربات اضافه کنید'
   end
  end
